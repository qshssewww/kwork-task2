...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    http_response_code(405); // Отправляем код ошибки "Method Not Allowed"
    echo json_encode(['error' => 'Only POST requests are allowed']);
    exit();
}

// Проверяем, что запрос содержит корректный JSON
$request_body = file_get_contents('php://input');
$request_data = json_decode($request_body, true);

if ($request_data === null) {
    http_response_code(400); // Отправляем код ошибки "Bad Request"
...
$main_data = $request_data['main'];

if (!isset($main_data['url'], $main_data['city'], $main_data['lang'], $main_data['currency'])) {
    http_response_code(400); // Отправляем код ошибки "Bad Request"
    echo json_encode(['error' => 'Invalid request data']);
    exit();
}
...
// Проверяем, что данные корректны и генерируем ответ
if ($url === '/en/dubai/' && $city === 'dubai' && $lang === 'english' && $currency === 'usd') {
    $response = [
     "Header" => [
         "seo" => [
    ...
    http_response_code(404); // Отправляем код ошибки "Not Found"
